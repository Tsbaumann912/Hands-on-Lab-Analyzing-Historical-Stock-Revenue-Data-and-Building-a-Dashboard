...
def forecast_carry(carry: np.ndarray, cfg: EnsembleConfig) -> np.ndarray:
    """B: z-scored curve carry (convenience-yield proxy)."""
...
def forecast_gs_ratio_real_yield_fade(
    close: np.ndarray,
    returns: np.ndarray,
    inventory: np.ndarray,
    gs_ratio: np.ndarray,
    usd_ret_20d: np.ndarray,
    real_yield_chg: np.ndarray,
    cfg: EnsembleConfig,
) -> np.ndarray:
    """
    E: gold–silver ratio mean-reversion + fade stretched SI moves when real
    yields and DXY rise together; disabled in structural inventory-confirmed trends.
    """
    n = close.shape[0]
    ...
    out = np.where(np.isnan(z_px) & np.isnan(z_ratio), np.nan, out)
    return _clip(out, cfg.forecast_cap)
